...
static void core1_main(void) {
  multicore_lockout_victim_init();      // For more stability while writing FLASH
  //static int frame = 0;
  while (1) {
    //if (++frame % 100 == 0) {
...
static void command(uint8_t com, size_t len, const char *data) {
    ...
    while (spi_get_hw(SPI_DISP)->sr & SPI_SSPSR_BSY_BITS) {     // Wait for SPI
      tight_loop_contents();  
    }
    gpio_put(CS_PIN, 1);
}
...
static void JPEGCleanVRAM(uint8_t mode) {
    ...
    memset((uint8_t *)lineBuffA, color, 64 * 2);
    while (dma_channel_is_busy(st_dma)) {};
    uint8_t cmd = RAMWR;
    gpio_put(CS_PIN, 0);
    gpio_put(DC_PIN, 0); // command mode
...
#define JPEGVIEWTYPE (1) // 0: top, 1: center
...
void JPEGSetViewPage(uint8_t newpage) {
    ...
    if( newpage == 1) {
#if JPEGVIEWTYPE == 0
        command(VSCRSADD, 2, "\x00\x00");               // for page1
#else
...
void JPEGModeEnd() {
    JPEGWaitDma();
    command(CASET,4,"\x00\x00\x01\x3F");        // restore default value
    command(RASET,4,"\x00\x00\x01\x3F");
...
void JPEGModeChange(uint8_t mode) {
    ...
    if( mode == 0){
        if( JPEG_mode != 0) {
            JPEGCleanVRAM(0);    // initialize view page
            //command(DISPOFF,0,NULL);
            command(NORON,0,NULL);
            command(DISPON,0,NULL);
            JPEG_Viewpage = 0;
        ...
        if( JPEG_mode != 1) {
            JPEGCleanVRAM(1);    // initialize view page
            command(VSCRDEF, 6, "\x00\x00\x01\xE0\x00\x00");    // 0,480,0
#if JPEGVIEWTYPE == 0
            command(PLTAR,4,"\x00\x00\x00\xEF");                // 0,239
#else
            command(PLTAR,4,"\x00\x28\x01\x17");                // partial display 40-279
#endif
...
void JPEGClearScreen(uint8_t mode) {
    JPEGWaitDma();
    JPEG_override ++;
    JPEGCleanVRAM(mode);    // initialize view page
    JPEG_override --;
}
