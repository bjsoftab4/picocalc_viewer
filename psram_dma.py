...
class DmaPio:
    ...
    def read_start(cls, data, rx_sm):
        ...
        src = PIO0_BASE + RXF0 + 4 * rx_sm  # LSB byte
        dst = memoryview(data)
        cls.rxdma = rp2.DMA()
        dmactrl = cls.rxdma.pack_ctrl(inc_read = False, treq_sel = DREQ_PIO0_RX0 + rx_sm, size = 0)
        cls.rxdma.config(read=src, write=dst, count=len(dst), ctrl=dmactrl, trigger=True)
        #dmactrl = cls.rxdma.pack_ctrl(inc_read = False, treq_sel = DREQ_PIO0_RX2, size = 2, bswap = True)
        #cls.rxdma.config(read=src, write=dst, count=len(dst), ctrl=dmactrl, trigger=True)
        # countは「転送」単位であり、デフォルトでは4バイトのワード size=0だと１バイト
        # 完了を待つ
    ...
    def write_start(cls, data, tx_sm):
        ...
        cls.txdma.config(read=src, write=dst, count=len(src), ctrl=dmactrl, trigger=True)
        #dmactrl = cls.txdma.pack_ctrl(inc_read = False, treq_sel = DREQ_PIO0_RX2, size = 2, bswap = True)
        #cls.txdma.config(read=src, write=dst, count=len(dst), ctrl=dmactrl, trigger=True)
        # countは「転送」単位であり、デフォルトでは4バイトのワード size=0だと１バイト
        # 完了を待つ
